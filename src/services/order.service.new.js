...
class OrderService {
  ...
  async getHeldOrders(query = {}, branchId) {
    ...
    const items = heldOrders.map(h => {
      ...
      tax_amount: meta.tax_amount !== undefined ? Number(meta.tax_amount) : 0,
  ...
  async getHeldOrderById(id, branchId) {
    ...
    return {
      ...
      discount: (held.items && typeof held.items === 'object' && !Array.isArray(held.items)) ? (held.items.discount ?? null) : null,
      tax_amount: (held.items && typeof held.items === 'object' && !Array.isArray(held.items)) ? Number(held.items.tax_amount ?? 0) : 0,
  ...
  async createOrder(data, userId, branchId) {
    ...
    for (const item of items) {
      ...
      let variation = null;
      let unitPrice = item.unit_price || item.price || Number(product.sellingPrice);
    ...
    const order = await prisma.$transaction(async (tx) => {
      ...
      if (table_id && normalizedOrderType === 'dine_in') {
  ...
  async holdOrder(data, userId, branchId) {
    ...
    // Handle table - can be table_id or table object
    const finalTableId = table_id || (table && table.id) || null;
    ...
    return {
      ...
      table: held.table,
      subtotal: Number(held.subtotal),
      tax_amount: Number(held.items?.tax_amount ?? 0),
  ...
  async resumeHeldOrder(heldOrderId, userId, branchId) {
    ...
    const order = await this.createOrder(orderData, userId, branchId);

    // Return order with table details attached
    return {
      ...order,
      table: held.table || null  // Attach table details from held order
    };
  }
