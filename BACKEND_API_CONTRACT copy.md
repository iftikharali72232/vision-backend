...
> **⚠️ IMPORTANT FOR TAX HANDLING:**
> The resume endpoint MUST return complete tax information for each item:
> - `tax_rate` (number): Tax percentage (e.g., 15 for 15%)
> - `tax_type` (string): 'exclusive', 'inclusive', or 'none'
> - `tax_amount` (number): Calculated tax amount for the item
> 
> If `product` object is included, it should also contain `tax_rate` and `tax_type` as fallbacks.
